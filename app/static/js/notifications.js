...
function createNotificationMessage(notification) {
  ...
  switch (notification.type) {
    ...
    case 'group_invite':
      return `${senderName} invited to you ${notification.group_name}`;
    case 'invite_accepted':
      return `${senderName} accepted your invitation to ${notification.group_name}`;
    case 'admin_promotion':
      return `${senderName} promoted you as an admin in ${notification.group_name}`;
    default:
...
function removeNotificationFromDropdown(notificationId) {
  const notificationItem = document.querySelector(
    `li[data-notification="${notificationId}"]`
  );
  if (notificationItem) {
